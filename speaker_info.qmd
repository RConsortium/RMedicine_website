...
This page is intended for folks who have had their abstract accepted for a lightning talk or regular talk at R/Medicine. Congratulations! We are excited to welcome you as a presenter. 
...
-   **Powerpoint**. Download the template [here](https://docs.google.com/presentation/d/1yTwE2O9HuQWD5zJBklH0e-MlQNDmBAH_/edit?usp=sharing&ouid=109461157699922412522&rtpof=true&sd=true).
...
1.  Install the {rUM} package from CRAN:

``` r
remotes::install_cran("rUM")
```

2.  Initialize a new slide project that includes the RMed templates. Be sure to include `template = "rmed"`. A `slides` directory will be created in your current path as defined by `here::here()` unless you use the `path =` argument. This demonstration will create the directory evaluated by `here::here()`:
...
Please read and abide by our [Code of Conduct](Attend.qmd). We ask that speakers especially review this code of conduct and are inclusive in the **words and images** used during their presentation.
