...
2. Initialize a new slide project that includes the RMed templates. Be sure to include
`template = "rmed2025"`. A `slides` directory will be created in your current path as
defined by `here::here()` unless you use the `path = ` argument. This demonstration
will create the directory evaluated by `here::here()`:
